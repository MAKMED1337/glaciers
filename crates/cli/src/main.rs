...
async fn async_main() -> Result<(), AppError> {
    ...
    match cli.command {
        Commands::Abi {
            ...
            let abi_db_path =
                abi_db_path.unwrap_or_else(|| configger::get_config().main.events_abi_db_file_path);
        ...
        Commands::DecodeLogs {
            ...
            let log_path =
                log_path.unwrap_or_else(|| configger::get_config().main.raw_logs_folder_path);
            let abi_db_path =
                abi_db_path.unwrap_or_else(|| configger::get_config().main.events_abi_db_file_path);
        ...
        Commands::DecodeTraces {
            trace_path,
            abi_db_path,
        } => {
            let trace_path =
                trace_path.unwrap_or_else(|| configger::get_config().main.raw_traces_folder_path);
            let abi_db_path = abi_db_path
                .unwrap_or_else(|| configger::get_config().main.functions_abi_db_file_path);
...
fn parse_config_value(value: &str) -> configger::ConfigValue {
    let value = match value.to_lowercase().as_str() {
        ...
        // Numeric values
        _ if value.parse::<usize>().is_ok() => {
            configger::ConfigValue::Number(value.parse().unwrap())
        }
        // List values
        _ if value.contains(',') => configger::ConfigValue::List(
            value
                .replace("[", "")
                .replace("]", "")
                .split(',')
