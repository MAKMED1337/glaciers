...
pub fn match_logs_by_topic0_address(
    log_df: DataFrame,
    abi_df: DataFrame,
    ...
    let logs_left_join_abi_df = log_df
        ...
        .with_column(
            ...
            .alias("num_indexed_args"),
        )
        .join(
            abi_df.lazy(),
            [
                col(topic0_alias.as_str()),
                col(address_alias.as_str()),
...
pub fn match_logs_by_topic0(
    log_df: DataFrame,
    abi_df: DataFrame,
) -> Result<DataFrame, MatcherError> {
    let logs_1 = match_logs_by_topic0_address(log_df.clone(), abi_df.clone())?;
    let log_df_cols: Vec<Expr> = log_df
        .columns()
        .iter()
        .map(|s| col(s.name().as_str()))
        .collect();

    // Split the logs into matched and not matched in the first step
    let logs_address_matched = logs_1
        .clone()
        .lazy()
        .filter(col("full_signature").is_not_null())
        .collect()?;
    let logs_address_not_matched = logs_1
        .lazy()
    ...
    let abi_df = abi_df
        ...
        .agg([all().as_expr().first(), len().alias("signature_count")])
        ...
        .group_by(["hash", "num_indexed_args"])
        .agg([all().as_expr().first()])
        .drop(cols(["address", "signature_count"]));
...
/// # Returns
/// * `Result<DataFrame>` - DataFrame with matched signatures, or error
pub fn match_traces_by_4bytes(
    trace_df: DataFrame,
    abi_df: DataFrame,
) -> Result<DataFrame, MatcherError> {
    let traces_1 = match_traces_by_4bytes_address(trace_df.clone(), abi_df.clone())?;
    let trace_df_cols: Vec<Expr> = trace_df
        .columns()
        .iter()
        .map(|s| col(s.name().as_str()))
        .collect();
    let traces_address_matched = traces_1
        .clone()
        .lazy()
        .filter(col("full_signature").is_not_null())
        .collect()?;
    let traces_address_not_matched = traces_1
        .lazy()
        .filter(col("full_signature").is_null())
        .select(trace_df_cols);
