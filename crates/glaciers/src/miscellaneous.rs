...
pub async fn decode_df_using_single_contract(
...
) -> Result<DataFrame, MiscellaneousError> {
    // Download the ABI from Sourcify
    let client = Client::new();
    let response = client
        .get(format!(
            "https://repo.sourcify.dev/contracts/partial_match/1/{}/metadata.json",
            contract_address
        ))
        .send()
        .await?;
    ...
    let abi_value = json_response
        .get("output")
        .ok_or(MiscellaneousError::InvalidJsonResponse(
    ...
    let abi: JsonAbi = serde_json::from_str(&abi_value.to_string())
        .map_err(|e| MiscellaneousError::InvalidJsonResponse(e.to_string()))?;

    let contract_address = contract_address.to_lowercase();
    let address = Address::from_str(&contract_address)
        .map_err(|e| MiscellaneousError::InvalidAddress(e.to_string()))?;
