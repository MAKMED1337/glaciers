...
/// # Returns
/// * If successful, a DataFrame with the converted columns.
pub fn hex_string_columns_to_binary(
    df: DataFrame,
    decoder_type: &DecoderType,
) -> Result<DataFrame, PolarsError> {
    let (input_schema_datatype, input_schema_alias) = match decoder_type {
        DecoderType::Log => (
            get_config().log_decoder.log_schema.log_datatype.as_array(),
            get_config().log_decoder.log_schema.log_alias.as_array(),
        ),
        DecoderType::Trace => (
            get_config()
                .trace_decoder
...
/// # Returns
/// * If successful, a DataFrame with the converted columns.
pub fn abi_df_hex_string_columns_to_binary(
    mut abi_df: DataFrame,
) -> Result<DataFrame, PolarsError> {
    // Convert hash and address columns to binary if they aren't already
    let columns_to_convert = ["hash", "address"];

    for col_name in columns_to_convert {
        if abi_df.column(col_name)?.dtype() != &DataType::Binary {
            abi_df = abi_df
                .lazy()
...
pub fn read_df_file(path: &Path) -> Result<DataFrame, PolarsError> {
    ...
    if path_ext == Some(OsStr::new("parquet")) {
        ...
        CsvReader::new(File::open(path).map_err(|e| {
            PolarsError::ComputeError(ErrString::from(format!(
                "Error opening path {}: {}",
                path.display(),
                e.to_string()
        ...
        Err(PolarsError::ComputeError(ErrString::from(format!(
            "In the path {}, a file extension was not provided (csv or parquet)",
            path.display()
        ))))
    }
}
...
pub fn write_df_file(df: &mut DataFrame, path: &Path) -> Result<(), PolarsError> {
    ...
    match path.extension().and_then(|ext| ext.to_str()) {
        ...
        _ => Err(PolarsError::ComputeError(ErrString::from(format!(
            "In the path {}, a file extension was not provided (csv or parquet)",
            path.display()
        )))),
    }?;
...
impl StrDynSolValue {
    pub fn to_string(&self) -> Option<String> {
        match &self.0 {
            ...
            DynSolValue::Bytes(b) => Some(format!(
                "0x{}",
